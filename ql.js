...
module.exports.getEnvidBysalt = async (salt) => {
  const envs = await this.getEnvs();
  for (let i = 0; i < envs.length; i++) {	
	var tempptsalt = decodeURIComponent(envs[i].value.match(/(salt=[\w\-]+)/) && envs[i].value.match(/(salt=[\w\-]+)/)[1]);
  // console.log(tempptsalt);
	if(tempptsalt==salt){	
    	//return i;  可返回envid  
      // console.log(i);
		 return envs[i]._id;
